...
from customer_accounts import resolve_customer,lock_account,balance,allocate
...
@router.get('')
def customers(q:str='',user=Depends(get_current_user)):
    conn=get_connection()
    try:
        with conn.cursor() as c:
            c.execute('''SELECT a.customer_id,a.name,a.phone,COALESCE(sum(l.amount),0) AS balance
              FROM customer_accounts a LEFT JOIN customer_ledger l USING(customer_id)
              WHERE a.name ILIKE %s OR a.phone ILIKE %s GROUP BY a.customer_id ORDER BY lower(a.name),a.customer_id''',('%'+q+'%','%'+q+'%'))
